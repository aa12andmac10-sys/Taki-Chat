...
function isImageUrl(text: string) { return IMAGE_REGEX.test(text.trim()); }
...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    const { error } = await supabase.from("messages").insert({
      username, content: input.trim(), reply_to_id: replyTo?.id ?? null,
    });
    if (!error) { setInput(""); setReplyTo(null); }
  };
  ...
  const findMessageById = (id: string | null) => id ? messages.find((m) => m.id === id) : undefined;
  ...
  return (
    <div className="flex h-screen flex-col bg-white text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      <header className="flex items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
        <h1 className="text-sm font-bold tracking-wide">連絡用チャット（ログイン中: {username}）</h1>
            ...
            <div key={msg.id} className="group mb-4">
              {quoted && (
                <div className="mb-1 ml-1 border-l-2 border-zinc-300 pl-2 text-xs text-zinc-500 dark:border-zinc-700 dark:text-zinc-500">
                  ...
                  「{quoted.content.length > 30 ? quoted.content.slice(0, 30) + "..." : quoted.content}」
                </div>
              )}
              <div className="flex items-baseline gap-2">
                <span className="text-sm font-bold">{msg.username}</span>
                <span className="text-xs text-zinc-400 dark:text-zinc-500">{formatDate(msg.created_at)}</span>
              ...
              <div className="mt-0.5 whitespace-pre-wrap break-words text-sm leading-relaxed">
                {isImageUrl(msg.content) ? (
                  <img src={msg.content} alt="投稿画像" className="mt-1 max-h-72 max-w-xs rounded border border-zinc-200 dark:border-zinc-800" />
                ) : msg.content}
              </div>
            </div>
          );
      ...
      <footer className="border-t border-zinc-200 p-3 dark:border-zinc-800">
        {replyTo && (
          <div className="mb-2 flex items-center justify-between rounded border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-xs dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <button onClick={() => setReplyTo(null)} className="ml-3 text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200">✕</button>
          </div>
        )}
        <div className="flex items-end gap-2">
          <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown}
            placeholder="メッセージを入力（Enterで送信 / Shift+Enterで改行）" rows={1}
